...
const UserManageInfo = () => {
  ...
  const handleToggle = async (newStatus) => {
    // Convert to boolean
    const booleanStatus = Boolean(newStatus);
    setIsActive(booleanStatus);

    if (!manageUser?._id || manageUser._id.length !== 24) {
      console.warn("Invalid user ID:", manageUser?._id);
      toast.error("Invalid user ID. Cannot update status.");
      return;
    }

    try {
      const response = await fetch(`https://realstate-2.onrender.com/api/v1/user/status/${manageUser._id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ isActive: booleanStatus }), // Send as true or false
      });

      if (!response.ok) throw new Error('Failed to update user status');

      // Update Redux
      dispatch(updateUserStatus({ userId: manageUser._id, isActive: booleanStatus }));
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50 md:p-4 p-2">
      <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-900 dark:to-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl p-4 relative border border-gray-200 dark:border-gray-700">
        <div
          className="absolute top-4 right-4 bg-gray-200 dark:bg-gray-800 rounded-full p-2 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 cursor-pointer"
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 mx-2 mt-4">
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-gray-800 dark:to-gray-700 p-4 rounded-xl flex items-center gap-3 flex-1 shadow-sm justify-between">
            <div>
              <div className="text-xl flex items-center text-red-900 dark:text-red-200">
                <IoMdAdd />
